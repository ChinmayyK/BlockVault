...
def add_bytes(data: bytes, pin: bool = True) -> Optional[str]:
    """Upload raw bytes to IPFS and return the CID.

    Similar to ``add_file`` but operates on in-memory bytes so callers
    don't need to write a temporary file to disk first.
    """
    if not ipfs_enabled():
        return None
    api_url = current_app.config.get("IPFS_API_URL") or "/dns/localhost/tcp/5001/http"
    try:
        ...
        if client is None:
            return None
        import io
        result = client.add(io.BytesIO(data), pin=pin)  # type: ignore
        if isinstance(result, list):
            for item in reversed(result):
                if "Hash" in item:
                    return item["Hash"]
